...
# PLAYER_ROTATION_SPEED. For the thrust button, we use a vector which is easier than trigonometry.
# So we take the vector(PLAYER_SPEED, 0) and rotate it by what our rotation is (-self.rotation).
# We are using negative (-self.rotation) because we're rotating the vector in the opposite direction of
# the directon the player sprite is pointing so that it can match the rotation of our keys.  
    ...
    def get_keys(self):
        ...
        if keys[self.key_right]:
            self.rotation_speed = -PLAYER_ROTATION_SPEED
        if keys[self.key_up]:
            self.vel = vec(PLAYER_SPEED, 0).rotate(-self.rotation)
...
class Laser(pygame.sprite.Sprite):
    ...
    def update(self):
        self.pos += self.vel * self.game.dt
        self.rect.center = self.pos
        if pygame.time.get_ticks() - self.spawn_time > LASER_LIFETIME: # This code decides range of laser
            self.kill()
"""
Barrel():
